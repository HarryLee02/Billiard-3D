...
public class Online_CameraController : MonoBehaviour
{
    ...
    void Start()
    {
        gameManager = GameObject.FindGameObjectWithTag("GameController").GetComponent<Online_GameManager>();
    ...
    void Shoot()
    {
        if (gameObject.GetComponent<Camera>().enabled)
        {
            ...
            else if (isTakingShot)
            {
                ...
                {
                    photonView.RPC(nameof(RPC_ShootAction), RpcTarget.AllBuffered, null);
                }
            }
        }
    }
    [PunRPC]
    void RPC_ShootAction()
    {
        ShootAction();
    }
    void ShootAction()
